...
Un archivo:
```
python3 $PATH_AUTOCRACKEO/autocrackeo.py -i ntlm.hash -m ntlm -c quick_test.json -w custom_wordlist.dic -o results -e="--username"
````

Varios archivos secuencialmente:
```
...
```
python3 %PATH_AUTOCRACKEO%\autocrackeo.py -i %PATH_PROYECTO%\ntlm.hash -m ntlm -c quick_test.json -w %PATH_PROYECTO%\custom_wordlist.dic -o %PATH_PROYECTO%\results -e="--username"
```
